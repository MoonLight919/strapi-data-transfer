  ...
  async createReadableStreamForStep(this: PullHandler, step: Exclude<TransferStage, 'schemas'>) {
    const mapper = {
      ...
      assets: () => {
        ...
        const BATCH_MAX_SIZE = 1024 * 1024 * 200; // 1MB
        ...
        async function* generator(stream: Readable) {
          ...
          for await (const chunk of stream) {
            ...
            if (!hasStarted) {
              ...
              batch.push({ action: 'start', assetID, data: assetData, batchID });
              hasStarted = true;
            }

            for await (const assetChunk of assetStream) {
              // Add the asset data to the batch
              batch.push({ action: 'stream', assetID, data: assetChunk, batchID });
            ...
            hasStarted = false;
            batch.push({ action: 'end', assetID, batchID });
